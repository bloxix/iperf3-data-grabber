...
#                command line. Use 2x2 quotes "' '". Note this option has not been tested.     #
...
function parse_args()
{
  ...
  echo "parse_args arguments are: $@"
  
  while getopts 'c:C:p:x:b:t:P:o:r:Rv' OPTION
  do
    case $OPTION in
      c) cflag=1
...
function execute_mysql_cmd()
{
  ...
  sudo mysql --user="$DB_USER" --password="$DB_PASSWORD" --database="$DB_NAME" --execute="$mysql_cmd"  
}

################################################################################################
# execute mysql select command and output single value                                         #
################################################################################################
function execute_mysql_select_cmd()
{
  local mysql_cmd="$1"
...
################################################################################################
# Retrieve test configuration from database
...
IPERF3_LOCAL_RUN=$(execute_mysql_select_cmd "SELECT IPERF3_LOCAL_RUN from $table")
export IPERF3_CLIENT=$(execute_mysql_select_cmd "SELECT IPERF3_CLIENT from $table")
export IPERF3_CLIENT_USER=$(execute_mysql_select_cmd "SELECT IPERF3_CLIENT_USER from $table")
export IPERF3_SERVER=$(execute_mysql_select_cmd "SELECT IPERF3_SERVER from $table")
export IPERF3_SERVER_USER=$(execute_mysql_select_cmd "SELECT IPERF3_SERVER_USER from $table")
IPERF3_PORT=$(execute_mysql_select_cmd "SELECT IPERF3_PORT from $table")
IPERF3_PROTOCOL=$(execute_mysql_select_cmd "SELECT IPERF3_PROTOCOL from $table")
IPERF3_BANDWIDTH=$(execute_mysql_select_cmd "SELECT IPERF3_BANDWIDTH from $table")
IPERF3_TIME=$(execute_mysql_select_cmd "SELECT IPERF3_TIME from $table")
IPERF3_PARALLEL=$(execute_mysql_select_cmd "SELECT IPERF3_PARALLEL from $table")
IPERF3_OTHER=$(execute_mysql_select_cmd "SELECT IPERF3_OTHER from $table")
IPERF3_REVERSE=$(execute_mysql_select_cmd "SELECT IPERF3_REVERSE from $table")
IPERF3_TEST_RUN_DELAY=$(execute_mysql_select_cmd "SELECT IPERF3_TEST_RUN_DELAY from $table")
IPERF3_RUN_PING=$(execute_mysql_select_cmd "SELECT IPERF3_RUN_PING from $table")
IPERF3_EXIT_ON_JSON_ERROR=$(execute_mysql_select_cmd "SELECT IPERF3_EXIT_ON_JSON_ERROR from $table")
TEST_RUNS=$(execute_mysql_select_cmd "SELECT TEST_RUNS from $table")
...
echo "Analyzing arguments for -o parameter in $my_args"

if [[ $my_args  =~ -o[[:space:]]*\'(.*)\' ]] # matching pattern -o '...' escaping the right chars in bash can drive you crazy... :)
then
  echo "Found a match for the other args in $my_args"
  other_args=${BASH_REMATCH[1]}
  echo "$other_args"
else
  echo "-o parameter not present in $my_args"
fi
rem_args=$(echo $my_args | sed -e "s/-o[[:space:]]*'$other_args'//")
echo "The remaining string is: $rem_args"

parse_args $rem_args

IPERF3_ARGS="$IPERF3_ARGS $other_args"
