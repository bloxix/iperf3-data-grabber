...
IPERF3_LOCAL_RUN=1                     # Set to 0 to run the iperf client from the local machine
...
IPERF3_SERVER_USER='heiko'             # used to login to IPERF3_SERVER
...
PATH_TO_JQ='/home/heiko/jq-linux64'
...
function parse_args()
{
  ...
  while getopts 'c:C:p:x:b:t:P:o:r:R' OPTION
  do
    case $OPTION in
      c) cflag=1
      ...
      R) Rflag=1
         IPERF3_REVERSE=1
         ;;
	  ?) printf "Usage: %s: [-c host_ip] [-p port] [-x protocol] [-b bandwidth] [-t time] [-P Parallel] [-o other] [-r runs] [-R]\n" $(basename $0) >&2
         exit 2
         ;;
      esac
  done
...
function update_iperf3_start()
{
  ...
  local mysql_cmd="INSERT INTO "$table" (test_id,test_time,test_name,test_user,test_runs,args) VALUES (\""$test_id"\",\""$test_time"\","\"$test_name"\","\"$test_user"\","\"$test_runs"\","\"$args"\");"
...
do
  ...
  if [[ $TOTAL_JSON == *"error"* ]]; then
	...
	echo "JSON dumped at iperf3_error.json"
	echo "$TOTAL_JSON" > iperf3_error.json
	exit 1
  fi
